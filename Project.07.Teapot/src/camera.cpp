...
void Camera::OnKeyboard(unsigned char Key)
{
    ...
    case 'p':   //para projeção perspectiva ou ortogonal
         m_pos -= (m_target * m_speed);
        break;
    
    case 'r':   //para reset
         m_pos +=(m_target * m_speed * 2);
        break;
    }
}

void Camera::ScrollMouse(int dir){
    if (dir > 0)
    {
        // Zoom in
        m_pos += (m_target * m_speed * dir);
    }
    else
    {
        // Zoom out
...
void Camera::Update()
{
    ...
    // glRotatef(m_AngleH, 1.0, 0.0, 0.0);
    // glRotatef(m_AngleV, 0.0, 0.0, 1.0);
}
