...
void Models::createTableBuffer(Vertex Table[( NELEMENTS_TABLE * NVERTICES_CUBOIDE * 3)]/*,  Vertex origin, float color[4]*/)
{
    // Table  --------------------------------------------------------

    // Vector with connection with GPU
    //Vector3f Table[( NELEMENTS_TABLE * NVERTICES_CUBOIDE )];
    Vertex topTable[NVERTICES_CUBOIDE *3];

    // Local variables to use in adjusts before send the datas to GPU
    Vertex legsTable[NLEGS][NVERTICES_CUBOIDE *3];
    Vertex origin;
    ...
    for (int i = 0; i < NVERTICES * 3; i+=3) {
        if(color[0] == -1){
            Table[i]     = Vertex(topTable[i/3].pos.x, topTable[i/3].pos.y, topTable[i/3].pos.z);  
            Table[i + 1] = Vertex(topTable[i/3].pos.x, topTable[i/3].pos.y, topTable[i/3].pos.z);  
            Table[i + 2] = Vertex(topTable[i/3].pos.x, topTable[i/3].pos.y, topTable[i/3].pos.z);  
        ...
        for (int j = 0; j < NLEGS; j++) {
            Table[i + (j + 1) * NVERTICES * 3]     =  Vertex(legsTable[j][i/3].pos.x, legsTable[j][i/3].pos.y, legsTable[j][i/3].pos.z, color);
            Table[(i+1) + (j + 1) * NVERTICES * 3] =  Vertex(legsTable[j][i/3].pos.x, legsTable[j][i/3].pos.y, legsTable[j][i/3].pos.z, color);
            Table[(i+2) + (j + 1) * NVERTICES * 3] =  Vertex(legsTable[j][i/3].pos.x, legsTable[j][i/3].pos.y, legsTable[j][i/3].pos.z, color);
    ...
    this->Buffer = (Vertex *)calloc(NELEMENTS_TABLE * NVERTICES_CUBOIDE *3 , sizeof(Vertex));
    if (!this->Buffer) {
        cout << "Memory Allocation Failed";
        exit(1);
    }
        ...
        int i;
        for (i = 0; i < NELEMENTS_TABLE * NVERTICES_CUBOIDE *3 ; i++)
        {
            *this->Buffer = Table[i];
            this->Buffer++;
        }
    ...
    id = 1;
    sizeBuffer = NELEMENTS_TABLE * NVERTICES_CUBOIDE *3;
    ...
    for (int i = 0; i < NELEMENTS_TABLE; i++)
    {
        for (int j = 0; j < NINDEX_CUBOID; j++)
        {
            if (i == 0)
                ...
                *this->Indices =  IndicesCuboid[j] + i*NVERTICES *3;    // *3 para acrecentar os vetores normais
...
void Models::setNormalVectorPoints(){
   ...
   for(int k = 0; k < sizeIndices; k+=3){
    ...
    for(int k = 0; k < sizeIndices; k++){
        printf("Buffer[%k]d= %d \t normal.x = %f, normal.y = %f, normal.z = %f \n",k, this->Buffer[this->Indices[k]], this->Buffer[this->Indices[k  ]].normal.x, this->Buffer[this->Indices[k  ]].normal.y, this->Buffer[this->Indices[k  ]].normal.z );
    }
}
