...
static void RenderSceneCB()
{
    ...
    // clipping without Projection
    glUniformMatrix4fv(gProjection, 1, GL_TRUE, &Projection.m[0][0]);
    ...
    glEnableVertexAttribArray(0);
        ...
        Matrix4f WVP = View * TableTransformMatrix * World;
    ...
    glEnableVertexAttribArray(0);
        ...
        Matrix4f WVP2 =  View * icoTransfMatrix * World;
    ...
    glEnableVertexAttribArray(0);
        ...
        Matrix4f WVP3 = View * cubeTransfMatrix * World ;
    ...
    float PlaneEquation[] = { 1.0f, 1.0f, 0.0f, 0.0f };
    glUniform4fv(ClipPlane, 1, &PlaneEquation[0]);
...
static void CompileShaders()
{
    ...
    gProjection = glGetUniformLocation(ShaderProgram, "gProjection");
    if (gProjection == -1) {
        printf("Error getting uniform location of 'gProjection'\n");
        exit(1);
    }

    ClipPlane = glGetUniformLocation(ShaderProgram, "ClipPlane");
    if (ClipPlane == -1) {
        printf("Error getting uniform location of 'ClipPlane'\n");
        exit(1);
    }
