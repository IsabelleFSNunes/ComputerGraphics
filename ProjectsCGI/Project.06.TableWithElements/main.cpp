...
static void RenderSceneCB()
{
    ...
    Matrix4f FinalMatrix = Projection * Translation * RotationY;
    ...
    glDrawElements(GL_LINE_LOOP, NELEMENTS_TABLE * NINDEX_CUBOID, GL_UNSIGNED_INT, 0);
...
static void CreateVertexBuffer(Models m)
{
    ...
    for (i = 0; i < NELEMENTS_TABLE * NVERTICES_CUBOIDE; i++) {
        tableArray[i] = *m.Buffer;
        m.Buffer++;
    }
    m.Buffer -= i;

    glBufferData(GL_ARRAY_BUFFER, sizeof(tableArray), tableArray, GL_STATIC_DRAW);
    // glBufferData(GL_ARRAY_BUFFER, sizeof(m.Buffer), 0, GL_STATIC_DRAW);
    // // objects index separated
    // glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(m.Buffer), m.Buffer);

}

static void CreateIndexBuffer(Models m, int nShift)
{   
    m.createTableIndices();
    ...
    for (i = 0; i < NELEMENTS_TABLE * NINDEX_CUBOID; i++) {
        tableArray[i] = *p;
        p++;
    }
